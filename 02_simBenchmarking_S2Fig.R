...
# null simulation
all_p <- read.csv("simulations/sim_results/sim_null/null_pval_all.csv", header = T, sep = ",")
...
ggsave("plots/supplementary/png/pval_hist.png", dpi = 500, width = 8.0, height = 5.0, plot = p_hist)
...
ggsave("plots/supplementary/png/qq_pval.png", dpi = 400, width = 8.0, height = 3.5, plot = qq_p)

# Performance metrics boxplot

# pipeline comparison
all_res_stager <- read.csv("simulations/sim_results/data_to_plot/performance_data_stageR_all.csv", header = T, sep = ",")
all_res_standard <- read.csv("simulations/sim_results/data_to_plot/performance_data_standard_all.csv", header = T, sep = ",")
...
strong_res <- all_res_stager %>% 
  filter(cn_signal == "strong")

metrics_long <- all_res_stager %>%
  dplyr::select(
    sample_size,
    replicate,
    pipeline,
...
metrics_summary <- metrics_long %>%
  group_by(sample_size, comparison, metric) %>%
  summarise(
...
ggsave("revision/plots/perf_boxp_pairwise.png", dpi = 400, width = 6.0, height = 4.0, plot = p_bxp)

# Confusion matrices

s20 <- read.csv("simulations/sim_results/data_to_plot/merged_S20_CNstrong_R2_standard.csv", header = T, sep = ",")
s60 <- read.csv("simulations/sim_results/data_to_plot/merged_S60_CNstrong_R2_standard.csv", header = T, sep = ",")
...
p_confusion = p1 / p2
p_confusion
ggsave("plots/supplementary/png/p_confusion_standard.png", 
...
read_data <- function(sim) {
  list(
    res_pydeseq = read.csv(paste0("simulations/sim_results/sim_res_fit/sim1/", "res_CNnaive_S60_G5000_CNstrong_R2.csv")),
    res_deconveil = read.csv(paste0("simulations/sim_results/sim_res_fit/sim1/", "res_CNaware_S60_G5000_CNstrong_R2.csv")),
    truth_df = read.csv(paste0("simulations/sim_results/sim_res_fit/sim1/", "truth_S60_G5000_CNstrong_R2.csv"))
  )
}
...
ggsave("plots/supplementary/png/sankey_sim1.png", dpi = 400, width = 4.0, height = 3.0, plot = p)
saveRDS(data_flow, file = "plots/supplementary/rds/tr_table_sim1.RDS")
